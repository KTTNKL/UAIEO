...
//THEM
                ...
                if( num<arr.size){
                    ...
                    if(num==arr.size){
                        ...
                        if (isOneQuestion == false) {
                            Toast.makeText(this@Part4, "Part 4: " + correctAnswers.toString() + "/" + arr.size, Toast.LENGTH_SHORT).show()
    ...
    private fun checkExist() {
        var auth = FirebaseAuth.getInstance()
        var curUID= auth.uid;
        var exits = FirebaseDatabase.getInstance().getReference("analyst/${exam.id}/${curUID}").child("part4")!!
        exits!!.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    reference.child("part4").setValue(0)
    ...
    private fun clickNext() {
        nextPart3Btn.setOnClickListener {
            ...
            if( num<arr.size){
                ...
                if(num==arr.size){
                    num=arr.size-3
                    Toast.makeText(this, "Part 4: " + correctAnswers.toString() + "/" + arr.size, Toast.LENGTH_SHORT).show()
                    nextPart3Btn.setText("XEM ĐIỂM")
                }
                else
                {
    ...
    private fun loadDataPart4() {
        val ref= FirebaseDatabase.getInstance().getReference("RLquestions").child("${exam.id}").child("part4")
        ref.addListenerForSingleValueEvent(object: ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                for (item in snapshot.children){
                    for( child in item.children){
